 ...
 */
function readTerms(path: string): {
  terms: Array<Term>
  repaired: number
} {
...
line(
  `roles:       ${ROLES.map(r => `-${r.vowel} ${r.name}`).join(', ')}`,
)
...
line(
  `  ${
    [...seen.values()].filter(v => v.length > 1).length
  } terms carry more than one meaning`,
)
...
line('| :-------- | :------- | ----: |')
for (const [shape, n] of Object.entries(shapeCount).sort(
  (a, b) => b[1] - a[1],
)) {
  const declared = SHAPES.includes(shape) ? 'yes' : 'no'
  line(
    `| ${shape.padEnd(9)} | ${declared.padEnd(8)} | ${String(
      n,
    ).padStart(5)} |`,
  )
}
...
line('')
line(
  `  ${onShape.length.toLocaleString()} terms are one of the eight shapes`,
)
line(`  ${offShape.length.toLocaleString()} are not`)
line(
  `  ${(
    (onShape.length / (onShape.length + offShape.length)) *
    100
  ).toFixed(1)}% of the lexicon fits`,
)

const offByShape: Record<string, Array<string>> = {}
for (const item of offShape) {
  offByShape[item.shape] = [
    ...(offByShape[item.shape] ?? []),
    item.term.term,
  ]
}
...
for (const [shape, words] of Object.entries(offByShape).sort(
  ...
  line(
    `    ${shape.padEnd(10)} ${String(words.length).padStart(
      4,
...
line('| :-------- | ----: |')
for (const n of Object.keys(syllableCount)
  .map(Number)
  .sort((a, b) => a - b)) {
  line(
    `| ${String(n).padEnd(9)} | ${String(syllableCount[n]).padStart(
      5,
    )} |`,
  )
}
...
function writeRows(path: string, rows: Array<Term>) {
  ...
  for (const item of sorted) {
    const gloss = item.meaning.includes(',')
      ? `"${item.meaning}"`
      : item.meaning
